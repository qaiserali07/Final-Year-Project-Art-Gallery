<?php
  ...
  $GetNotiCount = $conn->query("SELECT COUNT(*) AS `Total` FROM `tbl_notifications` WHERE `NID` = $ID AND `_Read` = -1");
  $GetNoti = $GetNotiCount->fetch_assoc();

  $getNotification = $conn->query("SELECT * FROM `tbl_notifications` WHERE `NID` = $ID AND `_read` = -1");
?>
